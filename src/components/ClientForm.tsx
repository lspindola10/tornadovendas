...
export default function ClientForm({ onSave, editingClient, onCancelEdit }: ClientFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.addressReference?.trim()) errors.addressReference = 'Ponto de referência é obrigatório.';
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="col-span-1 sm:col-span-3">
              <label htmlFor="addressReference-input" className="block text-xs font-semibold text-slate-700 mb-1">
                Ponto de Referência para Instalação <span className="text-orange-500">*</span>
              </label>
              <input
              ...
              {validationErrors.addressReference && (
                <p className="mt-1.5 flex items-start gap-1 text-[11px] font-medium text-red-500">
                  <AlertCircle className="w-3 h-3 mt-0.5" /> {validationErrors.addressReference}
                </p>
              )}
            </div>
          </div>
